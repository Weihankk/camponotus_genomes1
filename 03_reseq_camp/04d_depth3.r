x_files <- list.files(pattern="*depth.txt")[1:19]
...
rm(a_rep)
save.image("camp_coverage.RData")



load("camp_coverage.RData")

par(mfrow=c(4,5))
for(a in 1:length(output2)) {
	plot(0:61, output2[[a]], pch=19, cex=0.1, xlab="Coverage", ylab="Proportion Camponotus sp. genome", main=sample_names[a], ylim=c(0,0.08))
